...
# This lab presumes that the first 3 sections of the last script were run -
# the loader, the pruner (with the row-wise removing of missing values on)
# and the reformatter (putting ratings from all 3 movies into a "DATA"
# matrix). 
...
# A specified seed produces the same random numbers each time.
# In other words, the numbers will be 100% determined:
mySeed = 10 # This can be set to any number you want
random.seed(mySeed) # Initialize RGN
...
example = random.randint(1, 10)
print(example)
# Run this several times to see that the output is always the same

# Make sure to seed the random number generator with a random seed tied 
# to the system clock. Here is how to do that:
random.seed() # Now we are using the system clock
example = random.randint(1, 10)
print(example)
example = random.randint(1, 10)
print(example)
example = random.randint(1, 10)
print(example)
# Run this several times to see that the output changes each time
# This matters, for stuff like bootstrapping.
...
from idsLabCode8Spring2021 import combinedData
